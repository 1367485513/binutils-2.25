...
void
ldemul_before_parse (void)  /* Run before parsing the command line and script file.
...
void
ldemul_set_symbols (void) /* Run after assigning parsing with the args, but before
...
{
  ld_emulation_xfer_type **eptr = ld_emulations; /* Run before parsing the command line and script file.
     Set the architecture, maybe other things.  */
  /* Ignore "gld" prefix.  */
