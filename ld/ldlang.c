...
init_opb (void)
{
  unsigned x = bfd_arch_mach_octets_per_byte (ldfile_output_architecture,
					      ldfile_output_machine);  //This routine is provided for those cases where a bfd * is not available
  opb_shift = 0;
  if (x > 1)
    while ((x & 1) == 0)
      {
...
lang_process (void)
{
  ...
  if (link_info.dynamic_list)
    lang_finalize_version_expr_head (&link_info.dynamic_list->head); //版本，c，c++，java

  current_target = default_target;   // Get the default ARM target. 重要 重要 有reloc

  /* Open the output file.  */
  lang_for_each_statement (ldlang_open_output); /* lang_for_each_statement walks the 
  parse tree and calls the provided function for each node, except those inside output 
  section statements with constraint set to -1.  */

  init_opb ();  //octets_per_byte octet八位字节

  ldemul_create_output_section_statements ();//追踪创建section，不知咋创建的
  ...
  if (plugin_active_plugins_p ())
    {
	 ...
	 file.  */
      added = *stat_ptr;
      /* We need to manipulate操作 all three chains in synchrony同步.  */
      files = file_chain;
	...
	einfo (_("%P%F: %s: plugin reported error after all symbols read\n"),
      ...
      /* And detach分离 the fresh ends of the file lists.  */
  ...
  /* Find any sections not attached附上 explicitly and handle them.  */
  lang_place_orphans ();
  ...
  /* Do anything special before sizing sections.  This is where ELF
     and other back-ends size dynamic sections.  条件 */
  ...
  /* See if anything special should be done now we know how big
     everything is.  This is where relaxation is done. 重定位 分配   */
