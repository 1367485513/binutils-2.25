/* Main program of GNU linker.
   ...
   (at your option) any later version.中文
...
/* The canonical representation规范表示 of ld_sysroot.  */
...
main (int argc, char **argv) 
{
...
#ifdef HAVE_SBRK
  char *start_sbrk = (char *) sbrk (0); //分字节，堆到栈依次分配
#endif

#if defined (HAVE_SETLOCALE) && defined (HAVE_LC_MESSAGES)
  setlocale (LC_MESSAGES, "");    //196-203 每个binutils函数都会有。读取地域化信息
#endif
#if defined (HAVE_SETLOCALE)
  setlocale (LC_CTYPE, "");
#endif
  bindtextdomain (PACKAGE, LOCALEDIR); // 捆绑代码域
  textdomain (PACKAGE);

  program_name = argv[0];
  xmalloc_set_program_name (program_name); // 设置程序名称

  START_PROGRESS (program_name, 0);   // 找不到定义函数wenjian名

  expandargv (&argc, &argv); // 识别选项，存储选项在分配的buffer，并允许操作系统在程序退出时释放内存。

  bfd_init ();  // 二进制文件初始化,目前空函数

  bfd_set_error_program_name (program_name); // 设置打印BFD错误时要使用的程序名称

  /* We want to notice and fail on those nasty BFD assertions 令人讨厌的断言，言论 which are
     likely to signal incorrect output being generated but otherwise may
     leave no trace.  */
  default_bfd_assert_handler = bfd_set_assert_handler (ld_bfd_assert_handler); // 设置BFD断言处理函数。返回上一个函数。

  xatexit (ld_cleanup); // 注册(寄存器)函数FN由xexit运行。 如果成功则返回0，否则返回-1

  /* Set up the sysroot directory. 目录 */

  ld_sysroot = get_sysroot (argc, argv);  // Return the sysroot directory.  Return "" if no sysroot is being used.
  if (*ld_sysroot)
    {
      if (*TARGET_SYSTEM_ROOT == 0)  //TARGET_SYSTEM_ROOT为get_sysroot的return
	{
  ...
  /* Set the default BFD target based on the configured target.  Doing
     ...
     根据配置的目标设置默认BFD目标。 这样做允许为特定目标配置链接器，
     并链接到为不同目标配置的共享BFD库。 宏TARGET由Makefile定义。*/
  ...
  /* We initialize DEMANGLING based on the environment variable
     ...
     我们根据环境变量COLLECT_NO_DEMANGLE初始化DEMANGLING。 除非在环境中设
     置了COLLECT_NO_DEMANGLE，否则gcc collect2程序将对链接器的输出进行解码。
      在这里采取相同的方式让我们默认提供相同的接口。  */
  ...
  // 我们需要找到任何明确给定的仿真，以初始化lex＆yacc参数解析器（parse_args）所需的状态
  ldemul_choose_mode (emulation);
  default_target = ldemul_choose_target (argc, argv);
  config.maxpagesize = bfd_emul_get_maxpagesize (default_target); //最大页size
  config.commonpagesize = bfd_emul_get_commonpagesize (default_target);//通常页size
  lang_init (); /* Build enough state so that the parser can build its tree.  */
  ldemul_before_parse (); /* Run before parsing the command line and script file.
  ...
  ldemul_set_symbols ();  /* Run after assigning parsing with the args, but before
     reading the script.  Used to initialize symbols used in the script.  */

  /* If we have not already opened and parsed 解析 a linker script,
     try the default script from command line first.  */
  if (saved_script_handle == NULL
      && command_line.default_script != NULL)
    {
      ldfile_open_command_file (command_line.default_script); /* Open command file NAME.  */
  ...
  ldwrite ();  /* Call BFD to write out the linked file.  */
  ...
  if (link_info.notice_hash == NULL)
    {
      link_info.notice_hash =
          (struct bfd_hash_table *) xmalloc (sizeof (struct bfd_hash_table)); //sizeof为40字节
