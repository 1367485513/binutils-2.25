...
SECTION
	...
	BFD在BFD中保留一个原子，描述附加到BFD的数据的体系结构：指向<< bfd_arch_info_type >>的指针。

	可以独立于BFD请求指向结构的指针，以便可以在不访问开放BFD的情况下查询架构的信息。

	架构信息由每个架构包提供。
	宏<< SELECT_ARCHITECTURES >>选择了一组默认体系结构。 
	这通常在您选择的@file {config / @ var {target} .mt}文件中设置。 如果未定义名称，则包括支持的所有体系结构。

	当BFD启动时，使用initialize方法调用所有体系结构。 
	由架构后端根据需要将尽可能多的项插入到体系结构列表中;通常，这将是每台机器一个，
	默认情况下一个（机器字段为0的项）。

	BFD的架构概念在@file {archures.c}中实现。
*/
