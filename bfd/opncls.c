...
_bfd_new_bfd (void)
{
  ...
  nbfd = (bfd *) bfd_zmalloc (sizeof (bfd));  //分空间
  if (nbfd == NULL)
    return NULL;
  ...
  nbfd->memory = objalloc_create ();  //nbfd的memory成员建立结构
  if (nbfd->memory == NULL)
    {
  ...
  nbfd->arch_info = &bfd_default_arch_struct;  //nbfd的arch_info指针赋值地址

  if (!bfd_hash_table_init_n (& nbfd->section_htab, bfd_section_hash_newfunc,
			      sizeof (struct section_hash_entry), 13))  //13为哈希表的槽数
//【哈希表的section name，section哈希表的初始化一个entry，哈希表的element的size，为啥13呢】  这个函数return为true
    {
      free (nbfd); //释放nbfd所占的空间
      return NULL;
    }
...
/* Allocate a new BFD as a member of archive OBFD. 将新的BFD分配为归档OBFD的成员 */
...
_bfd_new_bfd_contained_in (bfd *obfd)
{
  ...
  nbfd->direction = read_direction;//选择读写或者both的direction
  nbfd->target_defaulted = obfd->target_defaulted;//标记在打开BFD时是否指定了默认目标。 这用于选择用于选择后端的匹配算法
  return nbfd;
}
...
bfd_fopen (const char *filename, const char *target, const char *mode, int fd)
{
  ...
  target_vec = bfd_find_target (target, nbfd); //翻译target
  if (target_vec == NULL)
    {
      if (fd != -1)
	close (fd);
  ...
  nbfd->filename = xstrdup (filename); //吧filename复制到内存
  ...
  if (fd == -1)
    (void) bfd_set_cacheable (nbfd, TRUE); //The new BFD is marked as cacheable iff @var{fd} is -1.
...
DESCRIPTION
	Create a new BFD in the manner 方式of <<bfd_openw>>, but without
