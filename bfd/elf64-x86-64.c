...
elf_x86_64_link_hash_table_create (bfd *abfd)
{
  ...
  ret = (struct elf_x86_64_link_hash_table *) bfd_zmalloc (amt);// 给链接哈希表table分空间
  if (ret == NULL)
    return NULL;
  ...
  if (!_bfd_elf_link_hash_table_init (&ret->elf, abfd,
				      ...
				      sizeof (struct elf_x86_64_link_hash_entry),
				      X86_64_ELF_DATA)) //Initialize an ELF linker hash table.*TABLE has been zeroed by our caller. 
    {
      free (ret); //初始化未成功执行此语句；
      return NULL;
    }

  if (ABI_64_P (abfd)) //判断架构位数
    {
  ...
  ret->loc_hash_table = htab_try_create (1024,
					 ...
					 NULL);
  ret->loc_hash_memory = objalloc_create (); //建立结构
  if (!ret->loc_hash_table || !ret->loc_hash_memory)
    {
      elf_x86_64_link_hash_table_free (abfd);//没有建立哈希表，就释放分配空间
      return NULL;
    }
