		       ...
		       unsigned int size)
{
  ...
  alloc = size;
  alloc *= sizeof (struct bfd_hash_entry *); //20个字节。对齐应为24字节
...
bfd_hash_hash (const char *string, unsigned int *lenp)
{
  ...
  while ((c = *s++) != '\0')
    {
      hash += c + (c << 17); //左移，加，自加
      hash ^= hash >> 2;  //右移，按位异或   不明白算法
    }
  len = (s - (const unsigned char *) string) - 1; // 判string长度
  hash += len + (len << 17);    
  hash ^= hash >> 2;
  if (lenp != NULL)
    *lenp = len;
